 ...
 */
public class CommandDefaultSDK implements Command {
    private Context context;

    private final int ThreadSleep = 650;
    ...
    public boolean setDensity(int density) {
        if(density < 72 && density != 0) {
            Log.d("CommandDefaultSDK", "Error: density must be >= 72");
            return false;
        }
        ...
        try {
            ...
            Log.d("CommandDefaultSDK", "COMMAND: " + cmd);
            Process p = r.exec(cmd);
            p.waitFor();

            String res = ProcessHelper.StreamToString(p.getInputStream());
            if(!res.isEmpty()) { Log.d("CommandDefaultSDK", "Process: \n" + res); }

            String resError = ProcessHelper.StreamToString(p.getErrorStream());
            if(!resError.isEmpty()) { Log.e("CommandDefaultSDK", "ProcessError: \n" + resError); }

            int retries = MaxRetries;
            while(retries > 0) {
                int dmDensity = context.getResources().getDisplayMetrics().densityDpi;
                Log.d("CommandDefaultSDK", "Retry: " + retries + " densityFromDM: " + dmDensity);
                Thread.sleep(ThreadSleep);

                //check if device is ok
                if(isCurrentDensity(density)) { return true; }

                retries--;
            }
        } catch (Exception ex) {
            Log.d("CommandDefaultSDK", ex.toString());
        }
    ...
    public boolean setSize(int width, int height) {
        try {
            ...
            while(retries > 0) {
                ...
                Log.d("CommandDefaultSDK", "Retry: " + retries + " dmWxH: " + dmWidth + "x" + dmHeight);
                Thread.sleep(ThreadSleep);

                // Check if device is ok
                if(isCurrentSize(width, height)) { return true; }
    ...
    public boolean setOrientation(Orientation orientation) {
        ...
        /*try {
            ...
            String res = ProcessHelper.StreamToString(p.getInputStream());
            if(!res.isEmpty()) { System.out.println("Process: \n" + res); }

            String resError = ProcessHelper.StreamToString(p.getErrorStream());
            if(!resError.isEmpty()) { System.err.println("ProcessError: \n" + resError); }
        } catch (Exception ex) {
            Log.d("CommandDefaultSDK", ex.toString());
        }*/
    ...
    public boolean isCurrentDensity(int density) {
        try {
            ...
            Log.d("CommandDefaultSDK", "Result: \n" + result + "\n\n");

            if(density == 0) {
                if(result.contains("Physical density: ") && !result.contains("Override density")) { return true; }
            } else if(!result.contains("Override density")) {
                if(result.contains("Physical density: " + density)) { return true; }
            } else {
    ...
    public boolean isCurrentSize(int width, int height) {
        try {
            ...
            } else if(!result.contains("Override size")) {
                if(result.contains("Physical size: " + width+"x"+height)) { return true; }
            } else {
